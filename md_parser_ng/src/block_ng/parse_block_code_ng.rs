...
use crate::{
    ...
    local_constants::{NEW_LINE_CHAR, CODE_BLOCK_END, CODE_BLOCK_START_PARTIAL, NEW_LINE},
};
...
mod tests_parse_code_block_body_including_code_block_end_ng {
    ...
    fn test_parse_code_block_body_simple_case() {
        ...
        {
            as_str_slice_test_case!(input, "fn main() {", "    println!(\"Hello\");", "}", "```");
    ...
    fn test_parse_code_block_body_special_characters() {
        ...
        {
            as_str_slice_test_case!(input, "#!/bin/bash", "echo \"$USER @ $(hostname)\"", "```");
            let result = parse_code_block_body_including_code_block_end_ng(input);
...
mod tests_compat_with_original_split_by_new_line {
    use super::*;
    use crate::{GCString, as_str_slice_test_case, assert_eq2};
    use r3bl_tui::split_by_new_line;
...
mod debug_tests {
    ...
    fn debug_test_parse_codeblock_multiple_lines() {
        ...
        let (remainder_after_body, body) =
            parse_code_block_body_including_code_block_end_ng(remainder_after_lang).unwrap();
