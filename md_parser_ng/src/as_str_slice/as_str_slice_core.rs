...
use r3bl_tui::{
    GCString,
    // Core types needed by this file
    core::units::{Index, Length},
    len,
};

use crate::as_str_slice::lazy_cache::LazyCache;
...
/// the nom trait signatures, this type alias is a way to "mark" that the `usize` in question in some
...
/// Wrapper type that implements nom input traits for &[`GCString`] or **any other type** that
...
///
/// This struct generates synthetic new lines when its nom input trait methods are used.
...
///
/// nom input traits use byte-based indexing, and `AsStrSlice`'s implementation of these
...
/// implementation of nom input traits which are byte index based. Literally everything else
...
/// ```
///
/// ## Compatibility with nom input traits
///
/// Since this struct implements nom input traits, it can be used in any function that can
/// receive an argument that implements them. So you have flexibility in using the
/// [`AsStrSlice`] type or nom input types where appropriate.
///
...
mod tests_as_str_slice_basic_functionality {
    ...
    use crate::{AsStrSlice, as_str_slice_test_case};
    use r3bl_tui::{assert_eq2, idx, len};
    ...
    fn test_with_limit() {
        ...
        let multi_line_slice = AsStrSlice::with_limit(&lines, idx(0), idx(6), Some(len(15)));
        ...
        let zero_limit_slice = AsStrSlice::with_limit(&lines, idx(0), idx(0), Some(len(0)));
...
mod tests_is_empty_character_exhaustion {
    use crate::{AsStrSlice, as_str_slice_test_case};
    use r3bl_tui::{GCString, assert_eq2, idx, len};
