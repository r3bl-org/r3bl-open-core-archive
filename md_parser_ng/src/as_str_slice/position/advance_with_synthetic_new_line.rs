...
//! library:
//! - Implements nom input traits for nom compatibility.
...
use crate::{local_constants::NEW_LINE_CHAR,
...
pub fn determine_position_state(this: &AsStrSlice<'_>, line: &str) -> PositionState {
    ...
    // Manual check since check_content_position is not available in older r3bl_tui
    let char_index = this.char_index.as_usize();
    let line_length = line.len_chars().as_usize();
    
    if char_index < line_length {
        PositionState::WithinLineContent
    } else if char_index == line_length {
        PositionState::AtEndOfLine
    } else {
        PositionState::PastEndOfLine
    }
