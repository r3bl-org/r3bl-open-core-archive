...
//!   documents
//! - **[`AsStrSlice`]**: Virtual array abstraction with nom input traits compatibility
...
//!
//! Since [`AsStrSlice`] implements nom input traits, any function expecting nom
//! input types can seamlessly accept [`AsStrSlice`]. This flexibility allows
...
pub use parse_markdown_simple::*;
pub use standard_ng::*;

// External dependencies and re-exports for internal use
pub use r3bl_tui::{
    BoundsCheck,
    BoundsStatus,
    BulletKind,
    // Nom and parsing types
    CheckboxParsePolicy,
...
// Re-export test data for tests
pub use compat_test_data::*;

// Local constants module
pub mod local_constants;

// Type alias for MdElement
pub type MdElement<'a> = r3bl_tui::MdBlock<'a>;

// bounds_check macro definition (copied from r3bl-open-core since older version doesn't export it)
#[macro_export]
...
// Module alias for self-references
pub use crate as md_parser_ng;
...
pub fn get_real_world_editor_content() -> &'static [&'static str] {
    const EX_EDITOR_CONTENT: &str = include_str!("compat_test_data/real_world_files/ex_editor.md");
...
macro_rules! as_str_slice_test_case {
    ($var_name:ident, $($string_expr:expr),+ $(,)?) => {
        ...
        let _input_array_binding = [$(r3bl_tui::GCString::new($string_expr)),+];
