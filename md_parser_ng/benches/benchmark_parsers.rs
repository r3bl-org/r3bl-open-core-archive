...
//!
//! This module contains performance benchmarks that compare three parsers:
...
//! across a wide variety of markdown content types and sizes.
//!
...
#[bench]
fn bench_a_small_empty_string_simple(b: &mut Bencher) {
    let content = EMPTY_STRING;
    b.iter(|| {
        let gcs_lines: Vec<GCString> = content.lines().map(GCString::from).collect();
        let _unused = parse_markdown_simple(&gcs_lines);
    });
}
...
#[bench]
fn bench_a_small_real_world_simple(b: &mut Bencher) {
    let content = SMALL_REAL_WORLD_CONTENT;
    b.iter(|| {
        let gcs_lines: Vec<GCString> = content.lines().map(GCString::from).collect();
        let _unused = parse_markdown_simple(&gcs_lines);
...
#[bench]
fn bench_a_small_simple_formatting_simple(b: &mut Bencher) {
    let content = MIXED_FORMATTING;
    b.iter(|| {
        let gcs_lines: Vec<GCString> = content.lines().map(GCString::from).collect();
        let _unused = parse_markdown_simple(&gcs_lines);
...
#[bench]
fn bench_b_medium_blog_post_simple(b: &mut Bencher) {
    let content = BLOG_POST_DOCUMENT;
    b.iter(|| {
        let gcs_lines: Vec<GCString> = content.lines().map(GCString::from).collect();
        let _unused = parse_markdown_simple(&gcs_lines);
...
#[bench]
fn bench_b_medium_code_blocks_simple(b: &mut Bencher) {
    let content = CODE_BLOCK_RUST;
    b.iter(|| {
        let gcs_lines: Vec<GCString> = content.lines().map(GCString::from).collect();
        let _unused = parse_markdown_simple(&gcs_lines);
...
#[bench]
fn bench_b_medium_nested_lists_simple(b: &mut Bencher) {
    let content = NESTED_UNORDERED_LIST;
    b.iter(|| {
        let gcs_lines: Vec<GCString> = content.lines().map(GCString::from).collect();
        let _unused = parse_markdown_simple(&gcs_lines);
...
#[bench]
fn bench_c_large_complex_document_simple(b: &mut Bencher) {
    let content = COMPLEX_NESTED_DOCUMENT;
    b.iter(|| {
        let gcs_lines: Vec<GCString> = content.lines().map(GCString::from).collect();
        let _unused = parse_markdown_simple(&gcs_lines);
...
#[bench]
fn bench_c_large_tutorial_simple(b: &mut Bencher) {
    let content = TUTORIAL_DOCUMENT;
    b.iter(|| {
        let gcs_lines: Vec<GCString> = content.lines().map(GCString::from).collect();
        let _unused = parse_markdown_simple(&gcs_lines);
...
#[bench]
fn bench_d_jumbo_api_documentation_simple(b: &mut Bencher) {
    let content = REAL_WORLD_EDITOR_CONTENT;
    b.iter(|| {
        let gcs_lines: Vec<GCString> = content.lines().map(GCString::from).collect();
        let _unused = parse_markdown_simple(&gcs_lines);
...
#[bench]
fn bench_d_jumbo_comprehensive_document_simple(b: &mut Bencher) {
    let comprehensive_input = get_real_world_editor_content().join("\n");
    b.iter(|| {
        let gcs_lines: Vec<GCString> =
            comprehensive_input.lines().map(GCString::from).collect();
...
#[bench]
fn bench_e_unicode_emoji_simple(b: &mut Bencher) {
    let content = UNICODE_CONTENT;
    b.iter(|| {
        let gcs_lines: Vec<GCString> = content.lines().map(GCString::from).collect();
        let _unused = parse_markdown_simple(&gcs_lines);
...
#[bench]
fn bench_e_unicode_emoji_headings_simple(b: &mut Bencher) {
    let content = EMOJI_START_MIDDLE_END;
    b.iter(|| {
        let gcs_lines: Vec<GCString> = content.lines().map(GCString::from).collect();
        let _unused = parse_markdown_simple(&gcs_lines);
...
// =============================================================================
// CHARACTER ACCESS PATTERN BENCHMARKS - Compare contiguous vs non-contiguous access
// =============================================================================
...
fn main() {
    println!("This benchmark must be run with `cargo bench`");
}
