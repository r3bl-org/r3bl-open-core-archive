...
//!
//! This module contains tests that ensure all three parsers produce identical output:
//! - Legacy parser (`parse_markdown`): Uses materialized strings
//! - NG parser (`parse_markdown_ng`): Uses nom with AsStrSlice virtual arrays  
//! - Simple parser (`parse_markdown_simple`): Direct string operations without nom
//! for all markdown inputs, including challenging edge cases.
//!
//! ## Parser comparison with real world use cases
//!
//! There are three main paths for parsing markdown in the R3BL TUI editor:
//! 1. Legacy parser path: &[`crate::GCString`] -> materialized string (full copy) ->
//!    [`crate::parse_markdown()`]
//! 2. NG parser path: Convert &[`crate::GCString`] to [`AsStrSlice`] (no copy) ->
//!    [`crate::parse_markdown_ng()`]
//! 3. Simple parser path: &[`crate::GCString`] -> direct parsing (no copy, no nom) ->
//!    [`crate::parse_markdown_simple()`]
//!
//! ## Module structure
//!
...
use md_parser_ng::compat_test_data::*;
#[cfg(test)]
use md_parser_ng::{get_real_world_editor_content, parse_markdown_ng,
                   parse_markdown_simple,
                   AsStrSlice, GCString, ParserByteCache};
#[cfg(test)]
use r3bl_tui::parse_markdown;

/// Helper function to test compatibility between `parse_markdown`,
/// `parse_markdown_ng`, and `parse_markdown_simple`. This simulates the real-world usage in
/// `try_parse_and_highlight` where all parsers start from the same &[`GCString`]
...
fn test_compat_helper(test_name: &str, input_str: &str) {
    ...
    // Step 4 - Simple parser path:
    // &[GCString] -> parse_markdown_simple(&[GCString])
    // Works directly with the slice without nom.
    let simple_doc = parse_markdown_simple(&gcs_lines);

    // Step 5 - Compare results:
    // All succeed → Compare their results
    // All fail → Test passes (consistent failure)
    // Mixed results → Test should fail with a clear message

    // All parsers should either succeed or fail consistently.
    assert_eq!(
        og_res.is_ok(),
        ng_res.is_ok(),
        // Panic message if assertion fails.
        "{a}: Legacy and NG parsers differ. Legacy: {b}, NG: {c}",
        a = test_name,
        b = og_res.is_ok(),
        c = ng_res.is_ok()
    );
    
    // Simple parser always returns a document (never fails), so we only check when legacy succeeds

    // Compare parser results
    match (og_res.is_ok(), ng_res.is_ok()) {
        (true, true) => {
            // Both legacy and NG succeeded - compare all three
            ...
            assert_eq!(
                    og_doc, ng_doc,
                    "{test_name}: Legacy and NG documents don't match.\nLegacy: {og_doc:#?}\nNG: {ng_doc:#?}",
                );
                
            // Compare with simple parser output
            assert_eq!(
                    og_doc, simple_doc,
                    "{test_name}: Legacy and Simple documents don't match.\nLegacy: {og_doc:#?}\nSimple: {simple_doc:#?}",
                );
        ...
        (false, false) => {
            // Both legacy and NG failed - test passes (consistent failure is valid).
            // Simple parser always succeeds, so we don't compare in this case.
        }
        _ => {
            // Legacy and NG parsers have inconsistent results.
            panic!(
                "{}: Legacy and NG parsers have inconsistent results. Legacy: {}, NG: {}",
                test_name,
                og_res.is_ok(),
                ng_res.is_ok()
            );
