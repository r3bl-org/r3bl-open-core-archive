...
pub mod tracing_config_options {
    ...
    impl Add<WriterConfig> for WriterConfig {
        ...
        fn add(self, rhs: WriterConfig) -> Self::Output {
            ...
            match (self, rhs) {
                ...
                (Display(_dp_lhs), DisplayAndFile(dp_rhs, f_rhs)) => DisplayAndFile(dp_rhs, f_rhs),
                ...
                // Collision (rhs has higher specificity). String can't be merged.
                (File(_f_lhs), DisplayAndFile(dp_rhs, f_rhs)) => DisplayAndFile(dp_rhs, f_rhs),

                // Collision (rhs has higher specificity). DisplayPreference can't be merged.
                (DisplayAndFile(_dp_lhs, f_rhs), Display(dp_rhs)) => DisplayAndFile(dp_rhs, f_rhs),

                // Collision (rhs has higher specificity). String can't be merged.
                (DisplayAndFile(dp_lhs, _f_lhs), File(f_rhs)) => DisplayAndFile(dp_lhs, f_rhs),
    ...
    mod tests_add_writer_configs {
        ...
        fn test_add_writer_configs() {
            ...
            let display_stdout_and_file = WriterConfig::DisplayAndFile(dp_stdout, fname.clone());
            let display_stderr_and_file = WriterConfig::DisplayAndFile(dp_stderr, fname.clone());
...
pub fn try_initialize_logging_global(arg_options: impl Into<TracingConfig>) -> miette::Result<()> {
...
pub fn file_log(file_path: Option<&Path>, message: &str) {
    let file_path = file_path.unwrap_or(Path::new(tracing_config_options::DEFAULT_LOG_FILE_NAME));
